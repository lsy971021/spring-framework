...
class ConditionEvaluator {
	...
	public boolean shouldSkip(@Nullable AnnotatedTypeMetadata metadata, @Nullable ConfigurationPhase phase) {
		// metadata 为空或者配置累中不存在 @Conditional 标签
		...
		if (phase == null) {
			...
			// 2、检查bean中是否使用@Configuration的注解
			// 3、检查bean不是一个接口
			// 4、检查bean中是否包含@Component @ComponentScan @Import @ImportResource中任意一个
			// 5、检查bean中是否有@Bean注解
			// 只要满足其中1，2 或者 1，3 或者 1，4 或者 1，5就会继续递归
		...
		for (Condition condition : conditions) {
			...
			if ((requiredPhase == null || requiredPhase == phase) && !condition.matches(this.context, metadata)) {
				// 此逻辑为：
				// 		1、requirePhase不是ConfigurationCondition的实例
				...
				// 如果1、2或1、3成立，则在此函数的上层将阻断bean注入Spring容器
				return true;
			}
		}

...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public ConfigurableEnvironment getEnvironment() {
		if (this.environment == null) {
			this.environment = createEnvironment(); //createEnvironment() ： 创建一个标准的系统环境
		}
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			try {
				...
				//子类覆盖方法做额外的处理，此处我们自己一般不作任何扩展工作，但是可以查看web中的代码，是有具体实现的
				postProcessBeanFactory(beanFactory);

				// Invoke factory processors registered as beans in the context.
				// 调用各种beanFactory的准备工作，对各种属性进行填充
				...
				// 为上下文初始化message源，即不同语言的消息体，国际化处理，在springmvc的时候通过国际化的代码有用
				initMessageSource();
				...
				//ApplicationListener：事件监听器，用于接收事件处理事件。接口。
				...
				// 发布事件 注入 ApplicationContext.publish(ApplicationEvent) 来发布事件
	...
	protected void prepareRefresh() {
		...
		//创建监听器集合,在springboot启动时，会有很多监听器（applicationListeners）
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		//为beanFactory增加一个默认的propertyEditor，这个主要是对bean的属性值类型等设置管理的一个工具类（对字段类型进行转换，如字段类型是Date,对设置的属性值（.properties中）进行转换）
		/**
		 * @see DateEditorRegistrar
		 */
		...
		// 设置几个自动装配的特殊规则，当在进行ioc初始化的如果有多个实现，那么就使用指定的对象进行注入（此处就是做个预防操作）
		// 和@Primary做的事情是一样的 （有多个匹配项时把哪个注入进去）
		...
		// 编译期织入是指在Java编译器，采用特殊编译器，将切面织入到java类中，
	...
	protected void initApplicationEventMulticaster() {
		...
		ConfigurableListableBeanFactory beanFactory = getBeanFactory();
		// 判断容器中是否存在bdName为 applicationEventMulticaster 的bd，也就是说自定义的事件监听多路广播器，必须实现 ApplicationEventMulticaster接口
	...
	protected void registerListeners() {
		...
		// 从容器中获取所有实现了 ApplicationListenerBeans 集合中
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		...
		// 为上下文初始化类型转换器
		// 最先初始化名字为 conversionService的类， 用来将前端传过来的参数和后端的 controller 方法伤的参数进行绑定的时候用
		// 尤其是用于非基础类型的转换
